...
function Guardar({ onClick }: { onClick: () => void }) {
...
export function TextsForm({ actuales }: { actuales: Record<string, string> }) {
  ...
  /*
    Cual fue el ultimo boton apretado. Todos guardan lo mismo —es un solo
    formulario— pero el aviso tiene que aparecer donde la persona estaba
    mirando, no al final de una pagina de setenta campos.
  */
      ...
      {GRUPOS.map((grupo) => (
        <section key={grupo.titulo}>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4 border-t border-line pt-6">
            <Guardar onClick={() => setUltimo(grupo.titulo)} />
            {state.message && ultimo === grupo.titulo && (
              <p
                role="status"
